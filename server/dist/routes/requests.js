...
router.get('/:id/negotiations', auth_1.authenticateToken, async (req, res) => {
    const userId = req.user?.id;
    const { id: requestId } = req.params;
    ...
    try {
        ...
        if (offerId)
            filters.offerId = String(offerId);
...
router.post('/:id/negotiations', auth_1.authenticateToken, async (req, res) => {
...
router.post('/notifications/mark-all-read', auth_1.authenticateToken, async (req, res) => {
    const userId = req.user?.id;
    if (!userId)
        return res.status(401).json({ message: 'Unauthenticated' });
    try {
...
router.post('/portfolio', auth_1.authenticateToken, async (req, res) => {
    const userId = req.user?.id;
    if (!userId)
        return res.status(401).json({ message: 'Unauthenticated' });
...
router.delete('/portfolio/:id', auth_1.authenticateToken, async (req, res) => {
    const userId = req.user?.id;
    const { id } = req.params;
    if (!userId)
        return res.status(401).json({ message: 'Unauthenticated' });
