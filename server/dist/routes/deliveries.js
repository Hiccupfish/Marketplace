...
router.post('/:jobId/offers', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!job)
            return res.status(404).json({ message: 'Delivery job not found or is no longer open' });
...
router.patch('/:jobId/offers/:offerId/accept', auth_1.authenticateToken, async (req, res) => {
